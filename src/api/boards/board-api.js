...
const url_read    = getServiceUrl(SERVICE_TYPES.BOARDS.BOARD_READ, 1) // Read API URL, Version
const url_command = getServiceUrl(SERVICE_TYPES.BOARDS.BOARD_CMD, 1) // Command API URL, Version
...
export default class BoardAPI {
  ...
  mapCreateFieldToApi(model, form_data){
    ...
    if(model.parent_id)
      form_data.append('parentId', model.parent_id)
  ...
  mapUpdateFieldToApi(model, form_data){
    form_data.append('boardId', model.id)
    form_data.append('title', model.title)
    form_data.append('contents', model.contents)
    form_data.append('masterOnlyReading', model.master_only_reading)
  ...
  async getBoardsAsync(query) {
    ...
    query_map.branchType = query.branch_type
    try {
  ...
  async updateBoardReadCountAsync(data) {
    let data_send = {
      boardId : data
    }

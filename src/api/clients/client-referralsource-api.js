...
const url_read    = getServiceUrl(SERVICE_TYPES.CLIENTS.CLIENT_CODESETUP_READ, 1) + CLIENTS_ENUMS.CLIENT_TYPE.CLIENT_REFERRAL_SOURCE
const url_command = getServiceUrl(SERVICE_TYPES.CLIENTS.CLIENT_CODESETUP_CMD, 1)  + CLIENTS_ENUMS.CLIENT_TYPE.CLIENT_REFERRAL_SOURCE

let url_change_order_no = url_command + SERVICE_EXTEND_TYPES.CHANGE_ORDER_NO
let url_list = url_read + SERVICE_EXTEND_TYPES.LIST

export default class ClientReferralSourceApi { 
  constructor(){ 
    this.http = new Http()
    this.result = {
      is_ok: false,
  ...
  async getClientReferralSourceAsync(query) {
    try {
  ...
  async addClientReferralSourceAsync(data) {
  ...
  async updateClientReferralSourceAsync(data) {
    let data_send = this.mapFieldToApi(data) 
    data_send.clientReferralSourceId =data.id 
    try {
  ...
  async updateClientReferralSourceOrderNoAsync(data) {    
    let data_send = {
      shopId        : data.shop_id,

...
const url_read    = getServiceUrl(SERVICE_TYPES.CLIENTS.CLIENT_CODESETUP_READ, 1) + CLIENTS_ENUMS.CLIENT_TYPE.CLIENT_RATING
const url_command = getServiceUrl(SERVICE_TYPES.CLIENTS.CLIENT_CODESETUP_CMD, 1)  + CLIENTS_ENUMS.CLIENT_TYPE.CLIENT_RATING

let url_change_order_no = url_command + SERVICE_EXTEND_TYPES.CHANGE_ORDER_NO
let url_list = url_read + SERVICE_EXTEND_TYPES.LIST

export default class ClientRatingApi { 
  constructor(){ 
    this.http = new Http()
    this.result = {
      is_ok: false,
  ...
  async getClientRatingAsync(query) {
    try {
  ...
  async addClientRatingAsync(data) {
  ...
  async updateClientRatingAsync(data) {
    let data_send = this.mapFieldToApi(data) 
    data_send.clientRatingId =data.id 
    try {
  ...
  async updateClientRatingOrderNoAsync(data) {    
    let data_send = {
      shopId        : data.shop_id,

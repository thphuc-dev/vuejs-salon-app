    ...
    >
      ...
      >
        <vgt-pagination
          ...
          :paginate-dropdown-allow-all="paginateDropdownAllowAll"
          :of-text="ofText"
          :page-text="pageText"
          :all-text="allText"
...
<script>
import {each,assign,cloneDeep,filter,isEqual} from 'lodash'
...
export default {
  ...
  props: {
    ...
    mode: { type: String, default: () => 'local' }, // could be remote
    rowKey: { type: String, default: () => 'id' },
    totalRows: {type: Number, default: () => 0 }, // required if mode = 'remote'
    styleClass: { type: String, default: () => 'vgt-table bordered' },
  ...
  methods: {
    ...
    toggleSelectAll() {
      ...
      each(rows, (headerRow) => {
        each(headerRow.children, (row) => {
          if(row.shared==undefined || row.shared!=true)
          {
    ...
    getClasses(index, element, row) {
      ...
      const classes = {
        ...
        'tal': this.typedColumns[index].left
      }

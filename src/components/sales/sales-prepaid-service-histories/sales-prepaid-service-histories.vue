      ...
      <view-table :data="table_data" @change-page="onChangePage">
        ...
        <template slot="mobile" slot-scope="{ row }">
          <div class="show-mobile-text-box">
            <p>
              <span>{{
                moment(convertTimeStampToDate(row.registration_date_ts)).format(
                  common_options.standard_date_format.ymd
                )
              }}</span>
              ...
              <span
                v-if="
                  row.client_name != null &&
                    row.client_name != x_client_information.data.client_name
                "
              >({{ formatNotes(row) }})</span
              >
            </p>
            <p
              v-if="
                row.expiry_date_ts_before_changed != row.expiry_date_ts &&
                  row.expiry_date_ts_before_changed != 0
              "
            >
        ...
        <template slot="notes" slot-scope="{ row }">
          <p
            v-if="
              row.client_name != null && row.client_name != x_client_information.data.client_name
            "
          >
            {{ formatNotes(row) }}
          </p>
...
export default {
  ...
  data() {
    return {
      ...
      table_custom_fields: [
        {
          field: 'branch_number',
          label: 'sales-prepaid-service-tab.loc',
          width: '5%',
          sortable: false,
          expand: true,
        },
      ...
      table_data: {
        fields: [
          ...
          {
            field: 'date',
            label: 'sales-prepaid-service-tab.date',
            width: '15%',
            sortable: false,
            expand: true,
          },
          {
            field: 'prepaid_service_name',
            label: 'sales-prepaid-service-tab.prepaid-service-name',
            width: '20%',
            sortable: false,
          },
          {
            field: 'unit_price',
            label: 'sales-prepaid-service-tab.sales-price',
            width: '10%',
            sortable: false,
            expand: true,
          },
          {
            field: 'action',
            label: 'sales-prepaid-service-tab.action',
            width: '10%',
            sortable: false,
            expand: true,
          },
          {
            field: 'changed_quantity',
            label: 'sales-prepaid-service-tab.changed',
            width: '10%',
            sortable: false,
            expand: true,
          },
          {
            field: 'quantity',
            label: 'sales-prepaid-service-tab.quantity',
            width: '10%',
            sortable: false,
            expand: true,
          },
          {
            field: 'notes',
            label: 'sales-prepaid-service-tab.notes',
            width: '20%',
            sortable: false,
            expand: true,
          },
  ...
  methods: {
    ...mapActions('sales_prepaid_service_history', ['loadSalesPrepaidServiceHistoriesAsyncData']),
    ...mapMutations('sales_prepaid_service_history', ['setPrepaidServiceHistoriesFilter']),
    ...
    formatAction(action) {
      let text = ''
      let prepaid_service_history_type = _.find(
        sales_options.prepaid_service_history_type,
        (t) => t.value == action
      )
    ...
    formatChanged(model) {
      ...
      if (
        typeof changed_quantity != 'number' ||
        typeof quantity_before_change != 'number' ||
      ...
      if (
        quantity_before_change == sales_options.NOT_LIMIT &&
        quantity != sales_options.NOT_LIMIT
      ) {
      ...
      text =
        after_changes > 0 ? `+${formatMoney(after_changes, 0)}` : formatMoney(after_changes, 0)
      return text
    },
    formatNotes(row) {

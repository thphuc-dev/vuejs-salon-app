<template>
  <main class="app-content">
    <section class="content report-sales-by-date-page">
      <div class="page-title">
        <h3>{{ $t('report.sales-by-date') }}</h3>
        <ul class="group-btn">
      ...
      <div class="filter-wrapper">
        <div class="row">
          <div class="col-12 col-sm-10 filter">
            <div class="row">
              <div class="col-12 form-group input-date-range">
      ...
      <div class="chart-wrapper">
        <template v-if="chart_has_data">
          ...
          <chart-line :key="chart_key" :data="chart_data" :is_decimal="true"/>
        </template>
        <template v-else>
...
export default {
  ...
  async mounted(){ // not created because input_date_range need render firsts
  ...
  methods:{
    ...
    async loadTableDataAsync(){
      ...
      this.preLoader()
      let response = await this.report_api.getSalesByDateReportAsync(this.table_filter)
      ...
      if(response.is_ok){
        ...
        // chart
        if(this.prepaid_sales_counting_type_loaded == sales_options.prepaid_sales_counting_type.sold)
          this.display_item_type_options = this.display_item_type_when_sold_options
        else
    ...
    formatPrepaidGoodsAmount(amount){
      let tmp = 0
      if(this.prepaid_sales_counting_type_loaded == sales_options.prepaid_sales_counting_type.used && amount == 0)
        tmp = '-'
      else
...
<style lang="scss">
@import './report-sales-by-date.scss';
</style>
